...
int colorCheck(char *board, int source);
...
int isRookMovable(char *board , char sourceCol, int sourceRow, char targetCol, int targetRow){
	...
	if (sourceRow==targetRow){	//if the rook is moving in the same row
		if (target>source){
			i+=1;		//moving right
			if (i==target){
				if (colorCheck(board,target)==-1){
					return 1;
				}
				else
				{
					if (colorCheck(board,source)==colorCheck(board,target)){
						return 0;
					}
					else
					{
		...
		if (target>source){
			...
			else
			{
				while (i<target){
					...
					i+=7;
					if(i==target){
						if (colorCheck(board,target)==-1){	//if there is nothing in that position
							return 1;
						}
						else
						{
...
int isPawnMovable(char *board,char sourceCol,int sourceRow,char targetCol,int targetRow){
	...
	else if (colorCheck(board,source)==0) {		// for black pawn
		if (source>target){
			return 0;
		}
	...
	else if (colorCheck(board,source)==1){	 //for white pawn
		if (target>source){
			return 0;
		}
...
int makeMove(char *board, char sc, int sr, char tc, int tr){
	...
	if(isPieceMovable(board,sc,sr,tc,tr) == 1){
		...
		if((isInCheck(board) == 1 && colorCheck(board, target) == 1) || 
			(isInCheck(board) == 2 && colorCheck(board, target) == 0)){  //it gives check 1,1 ->black king; check 2,0 ->white king	
			return 3;
		}
		if((isInCheck(board) == 1 && colorCheck(board, target) == 0) || 
			(isInCheck(board) == 2 && colorCheck(board, target) == 1)){  //it gives warning 
